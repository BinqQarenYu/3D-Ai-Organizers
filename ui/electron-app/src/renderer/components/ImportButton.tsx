...
const ImportButton: React.FC<ImportButtonProps> = ({ projectId, onImported }) => {
    ...
    const ACCEPTED = '.obj,.glb,.gltf,.fbx,.blend,.skp,.max,.rvt,.stl,.ply,.dae,.3mf,.3dm,.ifc';
        ...
        if (electronAPI?.selectFile) {
            ...
            try {
                const filePath: string | null = await electronAPI.selectFile({
                    ...
                    filters: [
                        { name: '3D Assets', extensions: ['obj', 'glb', 'gltf', 'fbx', 'blend', 'skp', 'max', 'rvt', 'stl', 'ply', 'dae', '3mf', '3dm', 'ifc'] },
                        { name: 'SketchUp / Revit / 3ds Max', extensions: ['skp', 'rvt', 'max'] },
